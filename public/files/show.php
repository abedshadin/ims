    ...
    try {
        ...
        if ($file === null) {
            ...
            $proformaStatement = $pdo->prepare(
                'SELECT id, invoice_number, pi_header, freight_amount, tolerance_percentage, created_at
            ...
            foreach ($proformaRows as $row) {
                ...
                if (is_array($file) && isset($file['bank_name'])) {
                    try {
                        $reference = ProformaReference::ensure($pdo, (int) $row['id'], (string) $file['bank_name']);
                ...
                $proformas[] = [
                    ...
                    'tolerance_percentage' => number_format((float) ($row['tolerance_percentage'] ?? 0), 2, '.', ''),
                    'tolerance_percentage_formatted' => number_format((float) ($row['tolerance_percentage'] ?? 0), 2),
                    ...
                    'reference' => $reference ? [
                        ...
                        'date' => $referenceDate,
                        'date_formatted' => $referenceDate ? date('j M Y', strtotime($referenceDate)) : null,
                    ] : null,
                ];
            }
...
$initialData = [
    'file' => $file ? [
        ...
        'beneficiary_bank_account' => (string) ($file['beneficiary_bank_account'] ?? ''),
...
<script type="module" src="../assets/js/file_invoices/index.js"></script>
</body>
</html>
